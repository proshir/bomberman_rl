"""Agent 032 training with allocation-light replay sampling.

The learning schedule and escape-replay quotas match Agent 030.  Only the
replay representation and batch handoff are changed here, so earlier agents
remain independent baselines.
"""
...
from agent_code.Agent_030_combat_ddqn_escape_replay_agent import train as _escape
from agent_code.combat_dqn_agent import callbacks as _base_callbacks
from agent_code.combat_dqn_agent import train as _dqn
from agent_code.combat_dqn_agent.callbacks import state_key
from agent_code.combat_dqn_agent.model import DEVICE, dqn_targets, load_checkpoint

from . import callbacks as _callbacks
from .features import ACTIONS
from .replay import CombatEscapeReplayBuffer
...
COMBAT_EPSILON_START = _escape.COMBAT_EPSILON_START
COMBAT_EPSILON_END = _escape.COMBAT_EPSILON_END
COMBAT_EPSILON_DECAY_STEPS = _escape.COMBAT_EPSILON_DECAY_STEPS
...
def optimize_model(self):
    ...
    # The replay buffer owns reusable CPU staging and device tensors.  This
    # avoids six fresh as_tensor allocations and six allocator/transfer paths
    # on every optimizer step.
    batch = self.replay_buffer.sample_torch(BATCH_SIZE, DEVICE)
...
def remember(self, old_state, action, new_state, events,
    ...
    reward = _dqn.reward_from_transition(
        old_state, action, new_state, events
    )
    ...
    self.replay_buffer.transition_tag = (
        "combat_escape" if is_combat_escape else None
    )
    if new_state is None:
        ...
        future = cached[0]
        next_action_mask = _dqn._candidate_mask(next_decision_state)
    else:
        future = _callbacks.next_features(
            self, old_state, action, new_state
        )
        next_action_mask = _dqn._candidate_mask(new_state)
