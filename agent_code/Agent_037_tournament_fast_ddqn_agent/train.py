"""Corrected, optimized DDQN training callbacks for Agent 037."""
...
from .features import ACTIONS, FEATURE_SIZE
...
def remember(self, old_state, action, new_state, events,
    ...
    if new_state is None:
        ...
        cached = self.feature_cache.get(_callbacks.state_key(next_decision_state))
        if cached is None:
            raise KeyError(
    ...
    self.replay_buffer.add(
        self.feature_cache[_callbacks.state_key(old_state)][0],
        ACTIONS.index(action), reward, future, new_state is None,
        next_action_mask,
    )
...
def end_of_round(self, last_game_state, last_action, events):
    if self.pending is not None:
        same_final = (
            ...
            _callbacks.state_key(self.pending[0]) == _callbacks.state_key(last_game_state)
        )
        if not same_final:
