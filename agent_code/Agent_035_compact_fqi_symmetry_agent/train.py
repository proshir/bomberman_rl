"""Scenario-balanced, symmetry-augmented fitted Q iteration for Agent 035."""
...
CHECKPOINT_SCHEMAS = (FEATURE_SCHEMA, SAFETY_SCHEMA, REWARD_SCHEMA,
                      HISTORY_SCHEMA, SYMMETRY_SCHEMA, "full-training-state-v2")
...
def balanced_records(transitions, weights=None, max_records=MAX_FIT_RECORDS):
    ...
    tag_weights = {tag: _tag_weight(tag, weights, lineups_per_scenario)
                   for tag in groups}
    tag_weights = {tag: weight for tag, weight in tag_weights.items() if weight > 0}
    if not tag_weights:
        raise ValueError("No positive replay weight for an available tag")
    scale = min(len(groups[tag]) / weight for tag, weight in tag_weights.items())
    scale = min(scale, max_records / sum(tag_weights.values()))
    result = []
    for tag in sorted(tag_weights):
        rows = groups[tag]
        count = max(1, min(len(rows), int(round(scale * tag_weights[tag]))))
...
def fit_trees(self):
    ...
    records = balanced_records(self.transitions, self.replay_weights)
    if not records:
        return
