"""Fit Bellman targets on collected transitions after each round."""
...
import events as e
from .callbacks import ACTIONS, predict_values, state_to_features, legal_action_indices
...
def remember(self, state, action, next_state, events):
    ...
    self.transitions.append((state_to_features(state), ACTIONS.index(action), reward,
                             state_to_features(next_state), legal))
...
def game_events_occurred(self, old_game_state, self_action, new_game_state, events):
    ...
    # The final action also reaches end_of_round: defer storing this transition.
...
def fit_trees(self):
    ...
    for _ in range(FIT_ITERATIONS):
        ...
        # All targets use the previous set of trees before any tree is replaced.
