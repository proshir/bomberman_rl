"""Agent 040 callbacks with per-state safety and feature reuse."""
...
from . import dep_Agent_040_optimized_compact_ddqn_agent_features as features
from .dep_Agent_040_optimized_compact_ddqn_agent_features import ACTIONS, StateContext
...
def _context_for_state(self, game_state):
    # The engine gives one state object per decision. Include object identity
    # so diagnostic callers can present different boards with the same
    # synthetic (round, step) pair without sharing stale board searches.
    key = (state_key(game_state), id(game_state))
    context = self._agent040_contexts.get(key)
    if context is None:
        context = StateContext(game_state)
        self._agent040_contexts[key] = context
    return context
...
def action_candidates(self, game_state):
    """Return the exact policy candidates, reusing the feature context."""
