"""Fitted-Q training for history-aware coin, crate, and combat behavior."""
...
import events as e
from .dep_combat_fqi_history_antistag_agent_callbacks import next_features, predict_values, state_key
from .dep_combat_fqi_history_antistag_agent_features import ACTIONS
from .dep_combat_fqi_history_antistag_agent_safety import (best_survival_action_indices, bomb_is_useful,
...
# These match combat_fqi_agent exactly.  Keeping reward fixed makes the pilot
# a controlled test of the history and stagnation representation.
COIN_REWARD = 1.0
KILL_REWARD = 5.0
CRATE_REWARD = 0.2
...
def reward_from_transition(old_state, action, new_state, events):
    """Use official outcomes plus the original small safety shaping terms."""
    reward = -STEP_COST
...
def end_of_round(self, last_game_state, last_action, events):
    """Store the true terminal transition, fit, and save the trees."""
    if self.pending is not None:
