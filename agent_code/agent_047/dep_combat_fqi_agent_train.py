"""Fitted Q iteration for coins, crates, survival, and combat."""
...
import events as e
from .dep_combat_fqi_agent_callbacks import predict_values
from .dep_combat_fqi_agent_features import ACTIONS, state_to_features
from .dep_combat_fqi_agent_safety import (best_survival_action_indices, bomb_is_useful,
...
def reward_from_transition(old_state, action, new_state, events):
    """Use official outcomes plus small, interpretable shaping terms."""
    reward = -STEP_COST
...
def remember(self, old_state, action, new_state, events):
    ...
    self.transitions.append((state_to_features(old_state), ACTIONS.index(action),
                             reward, state_to_features(new_state), legal))
...
def fit_trees(self):
    ...
    next_states = np.asarray([row[3] for row in self.transitions
                              if row[3] is not None])
    next_legal = np.asarray([row[4] for row in self.transitions
                             if row[3] is not None])
...
def end_of_round(self, last_game_state, last_action, events):
    """Store the true terminal transition, fit, and save a resumable model."""
    if self.pending is not None:
        pending_state = self.pending[0]
        pending_is_final = (
            last_game_state is not None and
            (pending_state["round"], pending_state["step"]) ==
