...
def surviving_followup_actions(game_state, action):
    """Return immediate moves that can escape a hypothetical bomb safely."""
    extra_bomb = action == "BOMB"
    danger = build_danger_schedule(game_state, extra_bomb)
    bombs = _bombs_with_times(game_state, extra_bomb)
    origin = tuple(game_state["self"][3])
    ...
    surviving = set()
    while queue:
        position, time_step, has_left, escaped_at, first_action = queue.popleft()
        if time_step >= horizon:
            if extra_bomb and escaped_at is not None and first_action is not None:
                surviving.add(first_action)
            elif not extra_bomb and first_action is not None:
                surviving.add(first_action)
            continue
        ...
        for next_action in ACTIONS[:5]:
            ...
            next_left = has_left or next_position != origin
            if not _tile_open_at(
                    game_state, next_position, next_time, origin, next_left,
                    bombs, opponent_reachability if escaped_at is None else None):
                continue
            if next_time in danger.get(next_position, ()):
                continue
            next_escaped_at = escaped_at
            if (extra_bomb and next_escaped_at is None and
                ...
                next_escaped_at = next_time
            next_first_action = (
                next_action if first_action is None else first_action
            )
            node = (
                next_position, next_time, next_left, next_escaped_at,
                next_first_action,
            )
            if node not in visited:
                visited.add(node)
