"""Tournament callbacks for history-aware, safety-masked combat FQI."""
...
from .dep_combat_fqi_history_antistag_agent_features import ACTIONS, HISTORY_LENGTH, state_to_features
from .dep_combat_fqi_history_antistag_agent_safety import best_survival_action_indices, safe_action_indices
...
def progress_signature(game_state):
    """Summarize meaningful game progress without choosing an action.

    A changed coin set includes collection and newly revealed coins.  Crate or
    opponent count changes and score changes also restart the stagnation clock.
    """
    return (
...
def setup(self):
    """Initialize the model and the episode-local movement memory."""
...
def features_for_act(self, game_state):
    """Build features and save exactly what training saw for this decision."""
...
def next_features(self, old_state, action, new_state):
    """Reconstruct the context that the following decision will observe."""
...
def act(self, game_state):
    """Choose a learned action from those with a predicted survival route."""
