"""Agent 038 callbacks for the 117-input wide route-feature teacher."""
...
from . import dep_Agent_038_symmetric_population_ddqn_agent_features as features
from .dep_Agent_038_symmetric_population_ddqn_agent_checkpoint import expand_agent037_checkpoint
from .dep_Agent_038_symmetric_population_ddqn_agent_features import ACTIONS
...
def setup(self):
    ...
    # The shared callback rejects a feature-width mismatch before loading.
    # Adapt only Agent 037's known 101-input format in memory, then restore the
    # shared loader so no other agent is affected.
    original_loader = _base.load_checkpoint
    if self.train and RESUME_PATH is not None:
        _base.load_checkpoint = lambda path: expand_agent037_checkpoint(
...
def _advance_action_context(self, game_state):
    ...
    if self.last_action is not None and self.last_action_state is not None:
        ...
        self.action_successes.append(int(action_is_legal(
            self.last_action_state, ACTIONS[self.last_action]
        )))
